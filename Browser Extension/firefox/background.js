...
function registerContextMenu() {
  Promise.resolve(menuAPI.remove(MENU_ID)).catch(() => {}).finally(() => {
    menuAPI.create({
      id: MENU_ID,
...
api.downloads.onCreated.addListener((downloadItem) => {
  ...
  Promise.resolve(api.downloads.cancel(downloadItem.id)).catch(() => {}).finally(() => {
    Promise.resolve(api.downloads.erase({ id: downloadItem.id })).catch(() => {});
  });
...
api.webRequest.onHeadersReceived.addListener(
  (details) => {
    ...
    if (contentDisposition || looksDownloadable(details.url, contentType)) {
      ...
      if (details.tabId >= 0) {
        api.tabs.get(details.tabId).then((tab) => {
...
async function sendToLocalApp(request) {
  try {
    ...
    notify(
      ...
      "Open the app first, then reload this Firefox extension and try again."
    );
    throw error;
  }
}
...
async function cookieHeaderFor(targetUrl) {
  try {
    const cookies = await api.cookies.getAll({ url: targetUrl });
